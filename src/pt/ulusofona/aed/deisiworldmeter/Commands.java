...
public class Commands {
    ...
    /*
    public static Result countRegions(String[] nomePaises) {
    Result resultado = new Result();
    String[] nomePaisesCalcular = nomePaises[1].split(",");
    int totalRegioes = countRegionsRecursively(nomePaisesCalcular, 0, 0, false);
    ...
    resultado.success = true;
    resultado.error = null;
    resultado.result = String.valueOf(totalRegioes);

    return resultado;
}

public static int countRegionsRecursively(String[] nomePaises, int index, int totalRegioes, boolean algumPaisValido) {
    // Caso base: Se chegamos ao fim da lista de países
    if (index >= nomePaises.length) {
        return algumPaisValido ? totalRegioes : -1;
    }

    String nomePais = nomePaises[index].trim();
    boolean paisEncontrado = false;

    // Iterar sobre a lista de países para encontrar o país atual
    for (Pais pais : Main.listaPaises) {
        if (pais.getNome().equalsIgnoreCase(nomePais)) {
            paisEncontrado = true;
            algumPaisValido = true;

            // Adicionar as regiões do país ao conjunto para contar apenas regiões únicas
            HashSet<String> regioesUnicas = new HashSet<>();
            for (Cidade cidade : pais.getCidades()) {
                regioesUnicas.add(cidade.getRegiao());
            }

            // Soma o número de regiões únicas desse país
            totalRegioes += regioesUnicas.size();
            break;
        }
    }

    // Chamada recursiva para o próximo país na lista
    return countRegionsRecursively(nomePaises, index + 1, totalRegioes, algumPaisValido);
}*/

    public static Result countRegions(String[] nomePaises) {
        Result resultado = new Result();
        // posicao 1 porque a posicao 0 é o COUNT_REGIONS
        String[] nomePaisesCalcular = nomePaises[1].split(",");
        int totalRegioes = 0;
        boolean algumPaisValido = false;

        for (String nomePais : nomePaisesCalcular) {
            boolean paisEncontrado = false;

            for (Pais pais : Main.listaPaises) {
                if (pais.getNome().equalsIgnoreCase(nomePais.trim())) {
                    paisEncontrado = true;
                    algumPaisValido = true;

                    // Incrementar o total de regiões para cada cidade do país
                    for (Cidade cidade : pais.getCidades()) {
                        totalRegioes += 1;
                    }
            ...
            // Se o país não for encontrado, simplesmente ignoramos e continuamos
            if (!paisEncontrado) {
                continue;
            }
        }

        // Se nenhum país válido foi encontrado, retornar "-1"
        if (!algumPaisValido) {
            return new Result(true, null, "-1");
        }
    ...
    /*
    public static Result getDensityBelow(double maxDensity, String nomePais) {
    // Procurar o país na lista de países disponíveis
    Pais paisEncontrado = null;
    for (Pais pais : Main.listaPaises) {
        if (pais.getNome().equalsIgnoreCase(nomePais.trim())) {
            paisEncontrado = pais;
    ...
    // Se o país não foi encontrado, retornar "Sem resultados"
    if (paisEncontrado == null) {
        return new Result(true, null, "Sem resultados");
    }
    ...
    getDensityBelowRecursively(paisEncontrado, maxDensity, 0, resultados);

    // Se não houver resultados, retornar "Sem resultados"
    if (resultados.isEmpty()) {
        return new Result(true, null, "Sem resultados");
    }

    // Ordenar os resultados alfabeticamente, por ordem decrescente
    resultados.sort(Comparator.reverseOrder());

    // Construir a string de resultado
    StringBuilder resultString = new StringBuilder();
    for (String resultado : resultados) {
        resultString.append(resultado).append("\n");
    }

    return new Result(true, null, resultString.toString().trim());
}

public static void getDensityBelowRecursively(Pais pais, double maxDensity, int index, ArrayList<String> resultados) {
    // Caso base: Se chegamos ao fim da lista de populações
    if (index >= Main.listaPopulacao.size()) {
        return;
    }

    Populacao populacao = Main.listaPopulacao.get(index);

    // Verificar se a densidade é menor que maxDensity e pertence ao país especificado
    if (populacao.getPais().getId() == pais.getId() && populacao.getDensidade() < maxDensity) {
        resultados.add(populacao.getAno() + " - " + populacao.getDensidade());
    }

    // Chamada recursiva para o próximo índice
    getDensityBelowRecursively(pais, maxDensity, index + 1, resultados);
}*/
    ...
    public static Result getDensityBelow(double maxDensity, String nomePais) {
        ...
        // Iterar sobre a lista de densidades populacionais
        for (Populacao populacao : Main.listaPopulacao) {
            if (populacao.getPais().getId() == paisEncontrado.getId() &&
                    populacao.getDensidade() < maxDensity) {
                resultados.add(populacao.getAno() + " - " + populacao.getDensidade());
            }
        }
        ...
        // Ordenar os resultados alfabeticamente, por ordem decrescente
        resultados.sort(String::compareTo);
