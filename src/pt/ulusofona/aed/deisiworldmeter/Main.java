...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean parseOK = parseFiles(new File("test-files/getDensity"));
    ...
    static final String COUNT_REGIONS = "COUNT_REGIONS";
    static final String GET_DENSITY_BELOW = "GET_DENSITY_BELOW";
...
public static Result getDensityBelow(double maxDensity, String nomePais) {
    String cacheKey = maxDensity + ":" + nomePais.toLowerCase().trim();
    if (cache.containsKey(cacheKey)) {
        return cache.get(cacheKey);
    }

    // Código original para calcular o resultado
    Result resultado = calcularDensityBelow(maxDensity, nomePais);
    ...
    */
    ...
    /*
    TreeMap<String, Cidade> cidadesTree = new TreeMap<>();
for (Cidade cidade : Main.listaCidades) {
    cidadesTree.put(cidade.getNome(), cidade);
}

public Cidade buscarCidade(String nomeCidade) {
    return cidadesTree.get(nomeCidade);
}
*/
    ...
    public static Result execute(String comando) {
        ...
        switch (comandoSplit[0]) {
            ...
            case GET_CITIES_AT_DISTANCE2:
                ...
                break;
            case COUNT_REGIONS:
                String[] comandoSplitCount = comando.split(" ");
                StringBuilder nomePaisBuilderCount = new StringBuilder();
                for (int i = 1; i < comandoSplitCount.length; i++) {
                    nomePaisBuilderCount.append(comandoSplitCount[i]).append(" ");
                }
                String nomePaisCount = nomePaisBuilderCount.toString().trim();
                Result countRegions = Commands.countRegions(new String[]{"COUNT_REGIONS", nomePaisCount});
                result.success = countRegions.success;
                result.result = countRegions.result;
                result.error = countRegions.error;
                break;
            case GET_DENSITY_BELOW:
                double maxDensity = Double.parseDouble(comandoSplit[1]);
                StringBuilder nomePaisBuilderDensity = new StringBuilder();
                for (int i = 2; i < comandoSplit.length; i++) {
                    nomePaisBuilderDensity.append(comandoSplit[i]);
                    if (i < comandoSplit.length - 1) {
                        nomePaisBuilderDensity.append(" ");
                    }
                }
                String nomePaisDensity = nomePaisBuilderDensity.toString();
                Result densityBelow = Commands.getDensityBelow(maxDensity, nomePaisDensity);
                result.result = densityBelow.result;
                result.error = densityBelow.error;
                result.success = densityBelow.success;
                break;
            case INSERT_CITY:
